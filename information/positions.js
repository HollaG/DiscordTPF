...
module.exports.getTop = (message, page, date) => {
    ...
    sql.all(`SELECT username, \`${pointValue}\` FROM scores ORDER BY \`${pointValue}\` DESC`).then(rows => {
        sql.get(`SELECT userId, \`${pointValue}\` FROM scores WHERE userId = '${message.author.id}'`).then(row => {
            //gets the total number of people
            while (rows[numberOfUsers]) {
                numberOfUsers++
            }
            ...
            function sendTop(message) {
                ...
                for (i; i < j; i++) {
                    ...
                    // error referring to rows[i] not defined => exceeded number of rows            
                    k = 0
                    if (!rows[i]) {
    ...
    }).catch(error => message.channel.send(`\`${date}\` is not a valid date! Please follow this format: !top [Month(shortform, 3 letters)_Year]. Example: \`!top nov_2017\`. \nFor more information, see \`!help top\``))
